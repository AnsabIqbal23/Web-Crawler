...
def save_images(soup, base_url, folder):
    ...
    for img in soup.find_all("img"):
        ...
        if src:
            img_url = urljoin(base_url, src)
            # Check if we're allowed to fetch this image
            if can_fetch(img_url):
                try:
                    img_data = requests.get(img_url).content
                    img_name = os.path.basename(src.split("?")[0])
                    with open(os.path.join(img_folder, img_name), "wb") as img_file:
                ...
                print(f"Skipping image {img_url} - disallowed by robots.txt")
...
def extract_text_and_images(url, folder="downloads"):
    # First check if we're allowed to crawl this URL
    if not can_fetch(url):
        print(f"Cannot crawl {url} - disallowed by robots.txt")
        return None
